...
def viralliset_pisteet():
    ...
    with open("tentin_aloitus.csv", "r") as file:
        for row in file:
            ...
            if len(parts) >= 2:
                ...
                points[name] = {}  # Initialize empty dictionary for each student
        ...
        for row in file:
            ...
            if len(parts) >= 4:
                ...
                if name not in cheaters:
                    task_no = int(parts[1])
                    task_points = int(parts[2])
                    submit_time = datetime.strptime(parts[3], '%H:%M')

                    # Check if submission is within time limit
                    if name in start_times and submit_time <= start_times[name] + timedelta(hours=3):
                        # Check if the task_no is in the acceptable range
                        if 1 <= task_no <= 8:
    ...
    for name in points:
        points[name] = sum(points[name].values())
